<?php  
    ...
    if ($timeRunningOut == $timeOut || $timeRunningOut > $timeOut) {
        ...
        mysqli_query($con, "UPDATE tahun_ajaran SET tahun = NULL, semester = NULL, status = NULL WHERE c_role = '$_SESSION[c_admin]' ");
    ...
    $dataTahun = mysqli_query($con, "SELECT * FROM tahun_ajaran WHERE status = 'aktif' AND c_role = '$_SESSION[c_admin]' ");
    $data_tahun = mysqli_query($con, "SELECT tahun FROM tahun_ajaran WHERE c_role = '$_SESSION[c_admin]' ");
    ...
    if ($countData != 0) {
        ...
        $queryDataTahun     = mysqli_query($con, "SELECT tahun FROM tahun_ajaran WHERE status = 'aktif' AND c_role = '$_SESSION[c_admin]' ");
        $queryDataSemester  = mysqli_query($con, "SELECT semester FROM tahun_ajaran WHERE status = 'aktif' AND c_role = '$_SESSION[c_admin]' ");
...
<script type="text/javascript">
    ...
    function refreshPage() {
        $.ajax({
            url : `<?= $basead; ?>tahunajaran`,
            type : 'post',
            data : {
                coba : "ok"
            },
            success:function(data){
                location.replace(`<?= $basead; ?>tahunajaran`)
            }
        })
    }
